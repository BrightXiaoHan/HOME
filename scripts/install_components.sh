...
homecli_install_frp() {
	local overwrite=${1:-true}
	local latest_version url archive_name extracted_dir tmp binary

	latest_version=$(homecli_latest_release_tag fatedier frp)
	if [ "$HOMECLI_ARCH" = "x86_64" ] || [ "$HOMECLI_ARCH" = "amd64" ]; then
		url="https://github.com/fatedier/frp/releases/download/$latest_version/frp_${latest_version#v}_linux_amd64.tar.gz"
	else
		url="https://github.com/fatedier/frp/releases/download/$latest_version/frp_${latest_version#v}_linux_arm64.tar.gz"
	fi
